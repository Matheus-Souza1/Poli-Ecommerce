...
class ClienteController {
    ...
    async store(req, res, next) {
        const { nome, email, telefones, endereco, dataDeNascimento, cpf, password } = req.body;
    ...
    async update(req, res, next) {
        ...
        try {
            ...
            if (password) cliente.setSenha(password);
            if (cpf) cliente.cpf = cpf
            if (telefones) cliente.telefones = telefones;
            if (endereco) cliente.endereco = endereco;
            if (dataDeNascimento) cliente.dataDeNascimento = dataDeNascimento;
    ...
    async remove(req, res, next) {
        try {
            const cliente = await (await Cliente.findOne({ usuario: req.payload.id })).populated("usuario");
