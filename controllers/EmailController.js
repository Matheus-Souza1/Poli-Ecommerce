...
const _send = ({ subject, emails, message }, cb = null) => {
    const mailOptions = { 
        from: "no-response@polipecas.com",
        to: emails,
        subject,
        html: message
    };
...
const enviarNovoPedido = ({ usuario, pedido }) => {
    const message = `
        ...
        <br /><br />
        <p>Atenciosamente,</p>
        <p>Equipe - Polipecas</p>
    `;
    _send({
        subject: "Pedido Recebido - Polipecas",
...
const cancelarPedido = ({ usuario, pedido }) => {
    ...
    `;
    _send({
        subject: "Pedido Cancelado - Polipecas",
        emails: usuario.email,
        message
    });
}
...
const atualizarPedido = ({ usuario, pedido, status, data, tipo }) => {
    ...
    `;
    _send({
        subject: "Pedido Atualizado - Polipecas",
        emails: usuario.email,
        message
    });
}
