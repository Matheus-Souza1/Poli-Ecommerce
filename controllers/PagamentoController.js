...
class PagamentoController {
    ...
    async show(req, res, next) {
        try {
            ...
            if (!pagamento) return res.status(400).send({ error: "Pagamento nao encontrado" });
            ...
            if (situacao && (registros.length === 0 || !registros[registros.length - 1].payload || !registros[registros.length - 1].payload.code || registros[registros.length - 1].payload.code !== situacao.code)) {
    ...
    async verNotificacao(req, res, next) {
        try {
            ...
            if (situacao && (registros.length === 0 || registros[registros.length - 1].payload.code !== situacao.code)) {
